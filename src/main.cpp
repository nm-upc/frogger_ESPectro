...
void drawMenu(int bestScore) {
    ...
    tft.setTextSize(4);
    const char* linia1 = "FROG";
    const char* linia2 = "GER";
...
void fgInitRow(int row) {
    ...
    if (row==0||row==6||row==12) return;
    bool isWater=(row>=1&&row<=5);
    int numV=(row%2==0)?3:2;
    for (int i=0;i<numV;i++) {
        fgVehicles[row][i].x = random(FG_COLS)*FG_CELL + FG_OX;
        fgVehicles[row][i].speed = random(1,3)*(row%2?1:-1);
        fgVehicles[row][i].w = isWater ? FG_CELL*2 : FG_CELL+8;
        fgVehicles[row][i].dir = (fgVehicles[row][i].speed>0);
        fgVehicles[row][i].color = isWater
            ? tft.color565(100+random(50),60,0)
...
void fgDrawRow(int row) {
    ...
    uint16_t bg = isSafe ? tft.color565(0,100,0) : (isWater ? tft.color565(0,30,120) : tft.color565(60,60,60));
    int y = FG_OY + row*FG_CELL;
    tft.fillRect(FG_OX, y, FG_COLS*FG_CELL, FG_CELL, bg);
    for (int i=0;i<FG_MAX_V;i++) {
        if (!fgVehicles[row][i].w) continue;
        int x=fgVehicles[row][i].x;
...
void fgDrawFrog(bool erase) {
    ...
    tft.fillRect(x+4, y+4, FG_CELL-8, FG_CELL-8, TFT_GREEN);
    tft.fillRect(x+6, y+6, 6, 6, TFT_WHITE);
